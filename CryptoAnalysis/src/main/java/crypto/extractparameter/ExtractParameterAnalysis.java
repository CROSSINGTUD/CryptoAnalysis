...
public class ExtractParameterAnalysis {
    ...
    public ExtractParameterAnalysis(Definitions.ExtractParameterDefinition definition) {
        Definitions.QuerySolverDefinition querySolverDefinition = new Definitions.QuerySolverDefinition(definition.callGraph(), definition.dataFlowScope(), definition.timeout(), definition.strategy());
        this.querySolver = new QuerySolver(querySolverDefinition);
    }

    public Collection<ParameterWithExtractedValues> extractParameters(Collection<Statement> statements, CrySLRule rule) {
    ...
    public Collection<ExtractParameterQuery> computeQueries(Collection<Statement> statements, CrySLRule rule) {
    ...
    private Collection<ExtractParameterQuery> getQueriesAtCallSite(Statement statement, CrySLMethod method) {
        Collection<ExtractParameterQuery> result = new HashSet<>();

        for (int i = 0; i < method.getParameters().size(); i++) {
            String parameter = method.getParameters().get(i).getKey();

            Collection<ExtractParameterQuery> queries = getQueriesForParameter(statement, i, parameter);
            result.addAll(queries);
        }
    ...
    private Collection<ExtractParameterQuery> getQueriesForParameter(Statement statement, int index, String parameter) {
        Collection<ExtractParameterQuery> result = new HashSet<>();
