...
public class CSVAGGReporter extends ErrorMarkerListener {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(CSVAGGReporter.class);
	...
	public CSVAGGReporter(String reportDir, String softwareId,  List<CrySLRule> rules, long callGraphConstructionTime) {
		this.reportDir = (reportDir != null ? new File(reportDir) : new File("."));
		this.rules = rules;
	...
	private void writeToFile() {
		try {
			FileWriter writer = new FileWriter(reportDir + File.separator+"CogniCrypt-CSVAGG-Report.csv");
			...
			writer.close();
			LOGGER.info("CSVAGG Report generated to location : "+ reportDir.getAbsolutePath());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
