...
public class CrySLModelReader {
    ...
    /**
     * Creates a CrySLModelReader which creates rules from classes on the runtime's classpath and a
    ...
    /**
     * Reads the content of a CrySL file from an {@link InputStream}, afterwards the {@link
    ...
    public CrySLRule readRule(InputStream stream, String virtualFileName)
            throws IOException, CryptoAnalysisException {
    ...
    private CrySLRule createRuleFromResource(Resource resource) throws CryptoAnalysisException {
        ...
        try {
            ...
            throw new CryptoAnalysisException(
                    "An error occurred while reading the rule " + resource.getURI(), e);
        }
    }
    ...
    private Collection<CrySLPredicate> getTimedPredicates(
        ...
        for (final TimedPredicate timed : timedPredicates) {
            ...
            ISLConstraint constraint =
                    timed instanceof ConditionalPredicate
        ...
        for (final TransitionEdge transition : this.smg.getAllTransitions()) {
            if (transition.getLabel().containsAll(condition)) {
                Collection<StateNode> reachableNodes =
                        getAllReachableNodes(transition.getRight(), Sets.newHashSet());
