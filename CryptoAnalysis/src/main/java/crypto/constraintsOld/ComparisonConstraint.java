...
public class ComparisonConstraint extends EvaluableConstraint {
    ...
    private Map<Integer, CallSiteWithExtractedValue> extractValueAsInt(
            ICrySLPredicateParameter par, CrySLArithmeticConstraint arith) {
        if (par instanceof CrySLPredicate) {
            PredicateConstraint predicateConstraint =
                    new PredicateConstraint((CrySLPredicate) par, context);
        ...
        try {
            ...
            try {
                for (Map.Entry<String, CallSiteWithExtractedValue> value :
                    ...
                    if (value.getKey().equals("true")) valuesInt.put(1, value.getValue());
                    else if (value.getKey().equals("false")) valuesInt.put(0, value.getValue());
                    else valuesInt.put(Integer.parseInt(value.getKey()), value.getValue());
                }
